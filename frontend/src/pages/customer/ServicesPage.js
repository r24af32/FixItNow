...
import { Search, SlidersHorizontal, MapPin, X, Loader2, Navigation } from 'lucide-react';
...
export const ServicesPage = () => {
      ...
      navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        try {
          ...
          if (data && data.address) {
            setLocationSearch(data.address.suburb || data.address.city || data.address.town || "My Location");
          }
  ...
  const handleProviderClick = async (service) => {
    ...
    try {
      ...
      const res = await fetch(`https://router.project-osrm.org/route/v1/driving/${mapLocation.lng},${mapLocation.lat};${service.providerLng},${service.providerLat}?overview=full&geometries=geojson`);
      const data = await res.json();
  ...
  const filtered = useMemo(() => {
    ...
    else if (sortBy === 'distance') list.sort((a, b) => (a.distance || '').localeCompare(b.distance || ''));
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex gap-3">
        ...
        <div className="bg-dark-800 border border-dark-700 p-4 rounded-xl mt-3 animate-fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end"> {/* Changed to sm:grid-cols-4 */}
            {/* Sort Dropdown */}
            <div>
              <label className="text-xs text-dark-400 font-medium mb-1.5 block">Sort By</label>
              <select 
                value={sortBy} 
      ...
      <div className="flex gap-3 mt-2">
        ...
        {/* LOCATE ME BUTTON */}
        <button onClick={handleLocateMe} className="bg-dark-800 text-brand-400 border border-dark-600 px-4 py-2 rounded-xl flex items-center gap-2 hover:border-brand-500 transition-all font-medium text-sm shrink-0">
          <Navigation className="w-4 h-4" /> <span className="hidden sm:inline">Locate Me</span>
        </button>
      </div>
        ...
        {['All', ...SERVICE_CATEGORIES.map(c => c.name)].map(cat => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all flex-shrink-0 ${
              selectedCat === cat ? 'bg-brand-500 text-white' : 'bg-dark-800 text-dark-400 hover:text-white border border-dark-700'
            }`}
          >
            {cat}
          </button>
      ...
      <div className="mt-4 relative">
        {routeDistance && (
          <div className="absolute top-4 right-4 z-[400] bg-dark-900 border border-brand-500 px-4 py-2 rounded-xl shadow-lg animate-fade-in">
            <p className="text-white text-sm font-semibold">📍Distance: <span className="text-brand-400">{routeDistance}</span></p>
          </div>
        )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-10 text-center">
          ...
          <p className="text-dark-400 text-sm">Try adjusting your map radius or search terms</p>
        </div>
      ) : (
  ...
  return (
    <div className="bg-dark-800 border border-dark-700 rounded-2xl overflow-hidden card-hover group">
      <div className="p-5 pb-4">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex flex-col items-end gap-1">
            {service.verified && <span className="badge bg-green-500/20 text-green-400 text-[10px]">✓ Verified</span>}
      ...
      <div className="border-t border-dark-700 px-5 py-3 flex items-center justify-between bg-dark-900/50">
        ...
        <Link to={`/customer/services/${service.id}`} className="btn-primary py-2.5 text-sm">Book Now</Link>
      </div>
    </div>
  );
};