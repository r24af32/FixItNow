...
export const ChatPage = () => {
  ...
  useEffect(() => {
    const loadRealContacts = async () => {
      ...
      try {
        ...
        res.data.forEach((b) => {
          if (user.role === "customer" && b.providerId)
            uniqueIds.add(b.providerId);
          if (user.role === "provider" && b.customerId)
            uniqueIds.add(b.customerId);
        });
        ...
        for (let id of uniqueIds) {
          try {
            ...
            loadedContacts.push({
              id: id,
              name:
                targetContact?.contactId === id
                  ? targetContact.contactName
                  : `User #${id}`,
              role:
                targetContact?.contactId === id
                  ? targetContact.contactRole
                  : user.role === "customer"
                    ? "Provider"
                    : "Customer",
              lastMsg: "Tap to chat",
        ...
        if (targetContact?.contactId) {
          const active = loadedContacts.find(
            (c) => c.id === targetContact.contactId,
          );
          ...
          prevContacts.map((contact) => {
            if (contact.id === receivedMessage.senderId) {
              return {
                ...
                unread:
                  activeConvIdRef.current === receivedMessage.senderId
                    ? 0
                    : (contact.unread || 0) + 1,
              };
            }
  ...
  const sendMessage = () => {
    ...
    setMessages((prev) => [
      ...prev,
      { ...chatMessage, id: Date.now(), sentAt: new Date().toISOString() },
    ]);
    setContacts((prev) =>
      prev.map((c) =>
        c.id === activeConv.id
          ? { ...c, lastMsg: `You: ${chatMessage.content}`, time: "Just now" }
          : c,
      ),
    );
    setInput("");
  ...
  if (contacts.length === 0)
    return (
      <div className="p-10 text-center text-white">
        No active chats yet. Go book a service!
      </div>
    );
  if (!activeConv)
    return <div className="p-10 text-center text-white">Loading chats...</div>;

  return (
    <div className="h-[calc(100vh-8rem)] flex bg-dark-950 rounded-2xl overflow-hidden border border-dark-700">
      {/* ─── SIDEBAR ─── */}
      <div
        className={`${showConvList ? "flex" : "hidden md:flex"} flex-col w-full md:w-80 bg-dark-900 border-r border-dark-700`}
      >
      ...
      {/* ─── CHAT WINDOW ─── */}
      <div
        className={`${!showConvList ? "flex" : "hidden md:flex"} flex-1 flex-col`}
      >
        <div className="flex items-center justify-between p-4 border-b border-dark-700 bg-dark-900">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setShowConvList(true)}
              className="md:hidden p-2 rounded-lg hover:bg-dark-700 text-dark-400"
            >
          ...
          <div className="flex items-center gap-1">
            <button className="p-2 rounded-xl text-dark-400 hover:bg-dark-700 transition-colors">
            ...
            return (
              ...
              >
                ...
                <div className={`ml-2 mr-2 max-w-[70%]`}>
                  <div
                    className={
                      isSelf ? "chat-bubble-sent" : "chat-bubble-received"
                    }
                  >
                    <p className="text-sm leading-relaxed">{msg.content}</p>
                  </div>
                  <p
                    className={`text-[10px] mt-1 text-dark-500 ${isSelf ? "text-right" : "text-left"}`}
                  >
