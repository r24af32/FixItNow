...
/**
 * ChatPage  –  Task 3 fixes:
 *
 * Fix 1: stompClientRef instead of useState for stompClient
 *        → sendMessage no longer captures a stale null reference
 * Fix 2: activeConvIdRef kept in sync on every activeConv change
 *        → incoming messages are correctly matched to the open chat
 * Fix 3: bottomRef.scrollIntoView called after every messages update
 *        → auto-scroll to latest message works reliably
 * Fix 4: WebSocket connect() only runs once (no user dependency re-run)
 *        → no duplicate subscriptions / connection leaks
 * Fix 5: Enter key send re-reads latest input via inputRef
 *        → no stale closure bug on keyDown
 */
export const ChatPage = () => {
  ...
  // Fix 1: keep stompClient in a ref so sendMessage never captures stale value
  const stompClientRef   = useRef(null);
  // Fix 2: ref always mirrors activeConv.id for WebSocket message routing
  const activeConvIdRef  = useRef(null);
  const bottomRef        = useRef(null);
  // Fix 5: ref mirrors input so keyDown handler reads latest value
  ...
  // ─── 3. WebSocket – Fix 4: runs once, no stale deps ─────────────────────
  useEffect(() => {
    if (!user?.id) return;
    ...
    client.connect({}, () => {
      client.subscribe(`/topic/messages/${user.id}`, (payload) => {
        ...
        // Fix 2: use ref for current chat check
  ...
  const sendMessage = useCallback(() => {
    ...
    // Instantly append own message – no waiting for server echo
    ...
    <div className="h-[calc(100vh-8rem)] flex bg-dark-950 rounded-2xl overflow-hidden border border-dark-700">
      ...
      <div className={`${!showConvList ? 'flex' : 'hidden md:flex'} flex-1 flex-col`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {/* Fix 3: scroll target always at bottom */}
          <div ref={bottomRef} />
        </div>
            ...
            <div className="flex-1 relative">
              <input
                ...
                onChange={e => {
                  ...
                  // Fix 5: keep ref in sync for keyDown handler
