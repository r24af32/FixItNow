...
import { MapContainer, TileLayer, Marker, useMapEvents, useMap, Popup, Polyline, Circle } from "react-leaflet";
    ...
    <>
      <Marker position={[activeLocation.lat, activeLocation.lng]} icon={userIcon}>
        <Popup className="custom-dark-popup">
          <b className="text-white">{isProviderView ? "Customer Location" : "Your Location"}</b>
        </Popup>
      </Marker>
...
export default function MapSelector({ services = [], onLocationSelect, mapLocation, onProviderClick, route, isProviderView = false }) {
  ...
  return (
    <>
      ...
      >
        ...
        {(!route || route.length === 0) && <ChangeView center={[currentLocation.lat, currentLocation.lng]} />}
        <FitBounds route={route} />
        <LocationMarker setLocation={onLocationSelect} currentLocation={mapLocation} isProviderView={isProviderView} />

        {/* 🔥 FIX 2: Stylish Glowing Orange Route! */}
        {route && route.length > 0 && (
          <>
            <Polyline positions={route} color="#fdba74" weight={8} opacity={0.4} />
            <Polyline positions={route} color="#ea580c" weight={4} opacity={1} />
          </>
        )}
                ...
                <div className="p-4 min-w-[220px]">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-brand-500/20 text-brand-400 rounded-full flex items-center justify-center font-bold text-lg">
                      {service.providerName ? service.providerName.charAt(0) : "P"}
                    </div>
                    <div>
                      <p className="font-bold text-white text-sm m-0 leading-tight">{service.providerName}</p>
                      <p className="text-xs text-dark-400 m-0">{service.category}</p>
                    </div>
                  </div>
                  ...
                  {!isProviderView && (
                    <div className="flex justify-between items-center mb-4 bg-dark-900/50 p-2 rounded-lg">
                      ...
                      <span className="font-bold text-brand-400">₹{service.price}</span>
                    </div>
                  )}
